...
//        UIImageView *shadowView = [[UIImageView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(frame)-7, TX_SCREEN_WIDTH, 10)];
//        shadowView.image = [UIImage imageNamed:@"shadow_line_down"];
...
- (void)titlesWidth {
    ...
    _space = (SCROLL_WIDTH-sum_width)/(titlesArray.count+1);
}

- (CGFloat)titleWidth:(NSString *)title {
    CGFloat sys_font = _fontScale>1?_fontSize*_fontScale:_fontSize;
    return [title boundingRectWithSize:CGSizeMake(MAXFLOAT, CGRectGetHeight(self.frame)) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName : [UIFont boldSystemFontOfSize:sys_font]} context:nil].size.width + _singleW_Add;
}
...
- (void)createBtn:(NSArray *)titlesArr addScroll:(UIScrollView*)scroll startX:(CGFloat)start_x start_index:(NSInteger)start_index {
    ...
    for (NSInteger i = start_index; i < titlesArr.count; i ++) {
        ...
        button.titleLabel.font = [UIFont boldSystemFontOfSize:_fontSize];
        button.frame = CGRectMake(start_x, 0, width, SCROLL_HEIGHT);
        start_x += (width+_space);
        if (titles) {
...
- (UIView *)lineView {
    ...
    line_w = [self titleWidth:titlesArray[currentIndex]];
...
//    line.layer.shadowOpacity = 0.8f;
//    line.layer.shadowRadius = 4.f;
...
- (void)changeBtnFrom:(NSInteger)from to:(NSInteger)to {
    ...
    if (_fontScale) {
        before_btn.titleLabel.font = [UIFont boldSystemFontOfSize:_fontSize];
        select_btn.titleLabel.font = [UIFont boldSystemFontOfSize:_fontSize*_fontScale];
    }
    
    if (lineView) {
        lineView.width = select_btn.width*_lineScale;
        lineView.width = [self titleWidth:titlesArray[currentIndex]];
...
- (void)fontChangeCurBtn:(UIButton *)curBtn nextBtn:(UIButton *)nextBtn changeScale:(CGFloat)changeScale{
    ...
    nextBtn.titleLabel.font = [UIFont boldSystemFontOfSize:next_font];
